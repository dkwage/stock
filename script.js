...
function populateModal() {
  ...
  document.getElementById('color-kospi').value =
    appState.indexColors['KOSPI'] || '#aaaaaa';
  document.getElementById('color-nasdaq').value =
    appState.indexColors['NASDAQ'] || '#34C759';
  document.getElementById('color-sp500').value =
    appState.indexColors['S&P 500'] || '#FF2D55';
  renderPortfolioList();
}
...
function renderPortfolioList() {
  ...
  if (appState.portfolios.length === 0) {
    list.innerHTML =
      '<p style="color: var(--text-secondary); font-size: 0.9rem;">No portfolios added.</p>';
    return;
  }
...
async function fetchTickerData(ticker, startDate) {
  ...
  const proxyUrl = `https://stock-proxy.kangminkoo0509.workers.dev/?url=${encodeURIComponent(yfUrl)}`;
...
async function initApp() {
  ...
  try {
    ...
    results.forEach((res) => {
      if (res.data) {
        ...
        Object.keys(res.data).forEach((d) => {
          if (d <= appState.endDate) {
            // filter out dates beyond end date in case proxy cached
            allDatesSet.add(d);
          }
        });
      } else {
    ...
    for (const [name, ticker] of Object.entries(INDICES)) {
      ...
      const baseDate = validDates.find(
        (d) => cleanedData[ticker]?.[d] !== undefined,
      );
...
function renderSummaryCards(pctChangeData) {
  ...
  for (const name of Object.keys(pctChangeData)) {
    ...
    const totalColor =
      currentReturn >= 0 ? 'var(--positive-color)' : 'var(--negative-color)';
    const dailyColor =
      dailyChangePct >= 0 ? 'var(--positive-color)' : 'var(--negative-color)';
...
function renderMonthlyTable(plotData) {
  ...
  thead.innerHTML =
    `<th>Date</th>` + columns.map((c) => `<th>${c}</th>`).join('');
